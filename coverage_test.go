...
func TestNewContainerFromViper(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "explicit logger", log: slog.New(slog.DiscardHandler)},
	}
...
func TestLoadFilesContainer_NoConfigFiles(t *testing.T) {
	...
	o := applyOptions([]ContainerOption{WithLogger(slog.New(slog.DiscardHandler))})
...
func TestHandleReadFileError_NonPathError(t *testing.T) {
	...
	c := &Container{logger: slog.New(slog.DiscardHandler)}
