...
import (
	...
	configMocks "github.com/phpboyscout/config"
)
...
func TestLoadEmbed(t *testing.T) {
	...
	t.Run("with empty embedded filesystem", func(t *testing.T) {
		...
		assert.Error(t, err)
		assert.Nil(t, container)
		assert.Contains(t, err.Error(), "failed to read embedded config file nonexistent.yaml")
	})
	...
	t.Run("with mock - successful single file read", func(t *testing.T) {
		...
		mockFS := configMocks.NewMockEmbeddedFileReader(t)
		yamlContent := []byte(`
server:
  port: 8080
  host: localhost
`)
		...
		mockFS.EXPECT().ReadFile("config.yaml").Return(yamlContent, nil).Once()
...
`)
		...
		mockFS.EXPECT().ReadFile("app.yaml").Return(config1, nil).Once()
		mockFS.EXPECT().ReadFile("db.yaml").Return(config2, nil).Once()
...
`)
		...
		// First file succeeds
		mockFS.EXPECT().ReadFile("logging.yaml").Return(validConfig, nil).Once()

		// Second file fails - function should return error on first failure
		mockFS.EXPECT().ReadFile("nonexistent.yaml").Return(nil, assert.AnError).Once()
	...
	t.Run("with mock - all files fail to read", func(t *testing.T) {
		...
		// First file fails to read - function should return error immediately
		mockFS.EXPECT().ReadFile("config1.yaml").Return(nil, assert.AnError).Once()
		...
		assert.Error(t, err)
		assert.Nil(t, container)
		assert.Contains(t, err.Error(), "failed to read embedded config file config1.yaml")
	})
...
`)
		...
		mockFS.EXPECT().ReadFile("complex.yaml").Return(complexYaml, nil).Once()
	...
	t.Run("with mock - empty file content", func(t *testing.T) {
		...
		// Return empty content (should still work)
		mockFS.EXPECT().ReadFile("empty.yaml").Return([]byte(""), nil).Once()
...
`)
		...
		mockFS.EXPECT().ReadFile("invalid.yaml").Return(invalidYaml, nil).Once()
